...
#definiton qui télécharge les scores dépuis le fichier et qui prend le top 10
def loadScores():
    ...
    with open(path) as f:
        for line in f:
            if isNotBlank(line):
                # Split les lignes pour avoir le score uniquement
    ...
    # Trie les scode du disctionnaire et ne garde que le top 10
...
#fonction qui permets de montrer les scores
def display_scores():
    top_scores = loadScores()  # Cette fonction retourne les scores
    top = Toplevel(window)
    top.geometry("500x400")
    Label(top, text="Top 10 Scores:", font=("Arial", 18)).pack()

    for player, score in top_scores:
        Label(top, text=f"{player} : {score}").pack()
...
def jeu():
   ...
   def verifier_lettre():
       ...
       #fonction pour fermer la fenetre en plus
       def close_win(w4):
           #Ficher sauvegarde
           script_path = Path(__file__).resolve()
           script_dir = script_path.parent
...
#fonction qui apparait si joueur gagne
   def popupwin():
#Creer la Toplevel window
...
#Cree un Entry Widget 
       entry= Entry(popupscore, width= 25)
       entry.pack()

#Creer un Button pr  imprimer ce qui est recu
...
#Create a Button Widget in the Toplevel Window
       ...
       def insert_val(e):
           ...
           _user = e.get()
           points_to_add = 2 if current_difficulty == "difficile" else 1
           
           #verifie si user existe deja et si oui lui mets les scores
           if _user in scores :            
               _val = scores[_user]
               print(_user)
               print(_val)
               scores[_user] = int(_val) + points_to_add
               loadScores()
           else:
               scores[_user] = points_to_add
               loadScores()
