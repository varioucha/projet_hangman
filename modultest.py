...
# définition qui créer et adapte le jeu (lettres trouvé et recherché)
 ...
 #bouton qui permet de vérifié la solution
    ...
    def ask_username():
        global username
        # Use a simple dialog to get the username
        username = simpledialog.askstring("Username", "Enter your username:")
        if username:  # Proceed only if the user entered a name
            play_action()
        else:
            messagebox.showinfo("No Username", "Please enter a username to play.")

    def update_score(is_win):
        """Update the leaderboard based on the outcome of the game."""
        ...
        if is_win:
            if username in scores:
                scores[username] += 1  # Increment score
            else:
                scores[username] = 1  # Initial win
    ...
    def display_leaderboard():
        ...
        Label(leaderboard_window, text="Leaderboard", font=("Arial", 24)).pack()

        for i, (user, score) in enumerate(sorted_scores, start=1):
            Label(leaderboard_window, text=f"{i}. {user} - {score}", font=("Arial", 18)).pack()
